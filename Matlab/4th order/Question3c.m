...
for i=1:length(Nx)
    ...
    for t=1:nt
        [wt{t+1}.w,wt{t+1}.F,wt{t+1}.G]=fRK44_4(w,dx,dy,dt,Ma,gamma);
        drawnow;
        surface(x,y,wt{t+1}.w{1});
        w=wt{t+1}.w;
    end
    ...
    for t=1:nt
        ...
        [wt]=fRK44_4(w,dx,dy,dt,Ma,gamma);
        %drawnow;
        %surf(x,y,wt{1});
        w=wt;
    end
    ...
    xlabel('x(units)');
    ylabel('y(units)');
    title({'Isentropic Vortex Using Fourth Order Central Spatial',['With Nx=' num2str(Nx(i)) ' and Ny=' num2str(Ny(i))]});
    saveas(gcf,['Contour_Vortex_4_Nx=' num2str(Nx(i)) '.jpg']);
    ...
    ylabel('y(units)');
    title({'Isentropic Vortex Using Fourth Order Central Spatial',['With Nx=' num2str(Nx(i)) ' and Ny=' num2str(Ny(i))]});
    saveas(gcf,['Surf_Vortex_4_Nx=' num2str(Nx(i)) '.jpg']);
    clf
...
ylabel('Error');
title({'Loglog Plot of the Error Versus Number of Grid Points', 'Using Fourth Order Central Spatial'});
saveas(gcf,['Error_Vortex_4_Nx=' num2str(Nx(i)) '.jpg']);
